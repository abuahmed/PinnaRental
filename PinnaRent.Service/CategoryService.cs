...
namespace PinnaRent.Service
{
    public class CategoryService : ICategoryService
    {
        ...
        {
            IEnumerable<CategoryDTO> catList;
            try
            {
                if (criteria != null)
                {
        ...
        public string Disable(CategoryDTO category)
        {
            ...
            try
            {
                //var items = new Repository<RoomDTO>(iDbContext).Query().Get();
        ...
        public int Delete(string categoryId)
        {
            try
            {
                ...
                _categoryRepository.Delete(catId);
                _unitOfWork.Commit();
                return 0;
            }
            catch
        ...
        public bool ObjectExists(CategoryDTO category)
        {
            ...
            try
            {
                ...
                var catExists = catRepository.Query()
                    .Filter(bp => bp.DisplayName == category.DisplayName && bp.Id != category.Id && bp.NameType == category.NameType)
                    .Get()
                    .FirstOrDefault();
                if (catExists != null)
                    objectExists = true;
