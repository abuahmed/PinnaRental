    ...
    public class UserViewModel : ViewModelBase
    {
        ...
        private void ExcuteGetNewPassword()
        {
            try
            {
                ...
                SelectedUser.Status=UserTypes.Waiting;
                if(string.IsNullOrEmpty(_userService.InsertOrUpdate(SelectedUser)))
                MessageBox.Show("Password Reset Successfully!");
                else MessageBox.Show("Problem on Resetting Password!");
            }
            catch
            {
        ...
        {
            Users = new ObservableCollection<UserDTO>(_userService.GetAll().Where(u => u.UserId > 1).ToList()); //
        }
